  ...
  static init(moduleOptions) {
   ...
   return initPromise = new Promise(resolveInitPromise => {
    ...
    function loadWebAssemblyModule(binary, flags, handle) {
     ...
     function loadModule() {
      var firstLoad = !handle || !SAFE_HEAP_LOAD(handle + 12 >> 0, 1, 0);
      if (firstLoad) {
       ...
       if (handle) {
        SAFE_HEAP_STORE(handle + 12 >> 0, 1, 1);
        SAFE_HEAP_STORE((handle + 16 >> 2) * 4, memoryBase, 4);
        SAFE_HEAP_STORE((handle + 20 >> 2) * 4, metadata.memorySize, 4);
        SAFE_HEAP_STORE((handle + 24 >> 2) * 4, tableBase, 4);
        SAFE_HEAP_STORE((handle + 28 >> 2) * 4, metadata.tableSize, 4);
       }
      } else {
       memoryBase = SAFE_HEAP_LOAD((handle + 16 >> 2) * 4, 4, 1);
       tableBase = SAFE_HEAP_LOAD((handle + 24 >> 2) * 4, 4, 1);
      }
